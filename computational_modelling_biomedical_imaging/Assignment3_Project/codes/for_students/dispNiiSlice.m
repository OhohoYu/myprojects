function h = dispNiiSlice(nii,dim,slice,clim,cmap,switch_dims,reorientate,use_vox_coords)
...
%               and -ve value used for srox_x(1), otherwise false)
...
if ~exist('reorientate','var') || isempty(reorientate)
    ...
    if nii.hdr.hist.sform_code > 0 && nii.hdr.hist.srow_x(1) < 0
        reorientate = true;
    else
        reorientate = false;
    end
