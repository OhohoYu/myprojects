...
author_data(dir.path, spec.file.name, loc.file.name, id.ind, yr.ind,auth.ind,
	    comma, in.tag, in.inc, ex.tag, ex.inc, tx.basio.filt, tx.basio.year,
            basio.ind, miss.bas, tx.tax.stat, stat.ind, stat.mk, tx.hyb.stat, 
            hyb.ind, hyb.mk, tx.rnk.stat, rnk.ind, rnk.mk, filt.ind, filt.mk, 
            loc.ind, levels, st.yr, en.yr, int.yr, rolling.windows, offset,
...
#
# if(grad.descent.cross.validation) {
#        cat("Computing Gradient Descent Search Model cross validation for", subsets[s],"...\n")
#        source("./kew_grasses/Support/model/grad_descent_search_log_residuals_cv.R")
#        grad_descent_search_log_residuals_cv(spec.data, tax.data, en.yr, mult, 
#                                          guess.n, ratio, stretch, max.it, scale,
#                                          rng.a, rng.b, ab.guesses, max.grad, alpha, 
#                                          min.alp, grad.rat, out.dir,id.str, 
#                                          mod.dir=grad.dir)
#        cat("Complete!\n\n")  
# }
