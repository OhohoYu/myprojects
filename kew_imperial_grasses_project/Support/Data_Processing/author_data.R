...
author_data <- function(dir.path, spec.file.name, loc.file.name=NULL, id.ind,
                        ...
                        basio.filt=FALSE, basio.year=FALSE, basio.ind=NULL, 
	...
	if(basio.year) {
	        ...
	        bas.auth <- numeric(length = nrow(spec.data))
	        bas.yr.ind <-  ncol(spec.data) + 1
	        bas.auth.ind <- ncol(spec.data) + 2
	        spec.data <- cbind(spec.data,bas.year,bas.auth)
	        names(spec.data)[bas.yr.ind] <- "basionym_year"
	        names(spec.data)[bas.auth.ind] <- "basionym_author"
	        rm(bas.year, bas.auth)
	        #
	        ...
	        spec.data[test,bas.auth.ind] <- spec.data[test,auth.ind]
	        #
	        # Copy dates and authors of basionyms for species that aren't 
	        # basionyms
	        #
	        ...
	        # bs.tmp <- table.merge(bs.tmp,spec.data,id=c(1,id.ind[1]),
	        #                      data.index = auth.ind, split = 2)
	        spec.data[test2,bas.yr.ind] <- bs.tmp[,2]
	        spec.data[test2,bas.auth.ind] <- bs.tmp[,3]
	        yr.ind <- bas.yr.ind
	        auth.ind <- bas.auth.ind
	        rm(bs.tmp,test2,test,bas.yr.ind, bas.auth.ind)
	}
	#
	...
	#
        if(replicate.research) {
	        tmp <- which(spec.data[,yr.ind] > research.year)
