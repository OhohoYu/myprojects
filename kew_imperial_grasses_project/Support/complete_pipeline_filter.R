...
for(s in 1:length(subsets)){
        ...
        author_data(tmp.dir, tmp.spec, tmp.loc, id.ind, yr.ind,auth.ind,
		    ...
		    tx.basio.year, basio.ind, miss.bas, tx.tax.stat, stat.ind, 
		    stat.mk, tx.hyb.stat, hyb.ind, hyb.mk, tx.rnk.stat, 
		    rnk.ind, rnk.mk, filt.ind, filt.mk, loc.ind, levels, st.yr, 
		    en.yr, int.yr, rolling.windows, offset, output.location, 
		    tax.dir, id.str)
        cat("Complete!\n\n")
        #
        ...
        #
        # Run complete grid search and steepest descent cross validation method for global
        # level if selected
        #
        if(grad.descent.cross.validation) {
                cat("Computing Gradient Descent Search Model cross validation for",
                     subsets[s],"...\n")
                source("./kew_grasses/Support/model/grad_descent_search_log_residuals_cv.R")
                grad_descent_search_log_residuals_cv(spec.data, tax.data, en.yr, mult, guess.n,
                                                  ratio, stretch, max.it, scale, rng.a, rng.b,
                                                  ab.guesses, max.grad, alpha, min.alp, 
                                                  grad.rat, out.dir,id.str, mod.dir=gradcv.dir)
                cat("Complete!\n\n")  
        }
