"""
Neural network model to classify MNIST digits consisting of 2 hidden layers (256
units each) with a ReLu non-linearity, followed by a softmax.
"""
# for compatibility
...
# question
subdirectory = "1c/"
model_filename = model_folder + subdirectory + "model_1c.ckpt"
...
# hidden layer 2
W_h2 = tf.Variable(0.1 * tf.random_normal([n_units_h1, n_units_h2]), name="W_h2")
b_h2 = tf.Variable(tf.random_normal([n_units_h2]), name="b_h2")
# add ReLu non-linearity to hidden layer 2
hidden2 = tf.nn.relu(tf.matmul(hidden1, W_h2) + b_h2)
# linear layer
W = tf.Variable(0.1 * tf.random_normal([n_units_h2, n_classes]), name="W")
b = tf.Variable(tf.zeros([n_classes]), name="b")
#  put together linear layer
z = tf.matmul(hidden2, W) + b
# softmax performed
...
with tf.Session() as sess:
    ...
    for iteration in range(tot_training_iterations):
        ...
        np.savez(os.getcwd() + "/traininghistory1C", train_loss_history=train_loss_history,
    ...
    plt.savefig('confmat1c.png')
    ############
    # learning curve plots for loss/error
    ############
    plt.figure()
    traininghistory = np.load(os.getcwd() + "/traininghistory1c.npz")
    ...
    plt.show()
    plt.savefig('1closslearningcurve.png')
    ###
    ###  learning curve for error
    ###
    plt.figure()
    traininghistory2 = np.load(os.getcwd() + "/traininghistory1c.npz")
    ...
    plt.xlim(0, tot_training_iterations)
    plt.show()
    plt.savefig('1cerrorlearningcurve.png')
...
with tf.Session() as sess:
    new_saver = tf.train.import_meta_graph(model_folder + subdirectory + "model_1c.ckpt.meta")
