clear all
%
% COMPGI07 ASSIGNMENT 2 - QUESTION 3 (GAUSSIAN)
%
...
for degree=2:max_degree
    ...
    train_kern=gaussian_kernel(train_set(:,2:end),train_set(:,2:end), degree);
    val_kern=gaussian_kernel(val_set(:,2:end),train_set(:,2:end), degree);
...
% Kernel computations
original_train_kern=gaussian_kernel(original_train(:,2:end),...
                                 original_train(:,2:end), optimal_degree);
test_kern=gaussian_kernel(test(:,2:end),original_train(:,2:end), optimal_degree);
...
disp('Confusion table for test set, d=6, 2 epochs: ')
...
for degree=2:max_degree
    ...
    test_kern=gaussian_kernel(test(:,2:end),original_train(:,2:end), degree);
    alpha_final = zeros(digitno, mot);
    % iterate over epochs
    for epoch=1:nepochs
        [tr_errors_final, alpha_final]=trainperceptron(original_train,...
...
fprintf('The 5 hardest-to-recognize digits over 6 degree iterations (2 epochs each) are: \n')
...
% these are plotted after inspecting the top5
figure; 
subplot(2,3,1)
...
subplot(2,3,6)
fprintf('True label of digit is %i\n\n', test(165,1))
imagesc(reshape(test(165,2:end), 16, 16)'); colormap 'gray';
