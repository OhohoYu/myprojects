...
data_dir = "Data/"
data_filename = "bandit4a_data.txt"
...
for index in range(0, 1): #model_no
    (params, POI, model_file, param_no) = BANDIT4A(model_names[index])
    ...
    for ind in range(0, len(param_configs)):
        ...
        for run in range(0, run_no):
            ...
            for k in range(0, len(data_splits)):
                ...
                rew = np.zeros((subject_no, max_trials))
                los = np.zeros((subject_no, max_trials))
                choice = np.ones((subject_no, max_trials), dtype = np.int)
                for subj in range(0, subject_no):
                    current_subj = subjects[subj]
                    use_Trials = Tsubj[subj]
                    tmp = data_splits[k][data_splits[k].values[:, subject_id_col-1] == current_subj]
                    rew[subj, :use_Trials] = tmp.values[:, rew_col-1]
                    los[subj, :use_Trials] = -1*np.abs(tmp.values[:, los_col-1])
                    for tIdx in range(0, use_Trials):
                        Y_t = tmp.values[tIdx, choice_col-1]
                        choice[subj, tIdx] = Y_t
                data_list = {'N': subject_no, 'T': max_trials, 'Tsubj': Tsubj, 'rew': rew, 'los': los,
                             'choice': choice, 'param_config': param_config, 'grouping': grouping}
                genInitList = BANDIT4A_genInitList(model_names[index])
                # Fit the STAN model
