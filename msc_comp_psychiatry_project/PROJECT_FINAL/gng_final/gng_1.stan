...
# gng_m1: RW + noise model in Guitart-Masip et al 2012
  ...
  if (param_config[1]==grouping) {
    xi_pr ~ normal(0, 1.0);
  } else {
    xi_pr ~ normal(0, 0.0001);          
  }
  
  if (param_config[2]==grouping) {
    ep_pr ~ normal(0, 1.0);          
  } else {
    ep_pr ~ normal(0, 0.0001);
  }
  
  if (param_config[3]==grouping) {
    rho_pr ~ normal(0, 1.0);          
  } else {
    rho_pr ~ normal(0, 0.0001);
  }  
